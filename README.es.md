...
| **S9** | una región que es la mañana de alguien | 🟢 **el bucle multi-turno cierra** — un cliente OpenAI con `tools=[…]`, el proxy, resultados devueltos como `role: "tool"`: 24 llamadas, **0 rechazadas, 0 sin decidir**. La suite no se puede responder desde el listado (mejor regla **0,680** contra una barra de **0,680**), y su verdad es **verificable mecánicamente** — un verificador sin juez |
