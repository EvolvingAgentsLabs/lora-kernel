...
| M43 | **A learned protocol DOES beat a hand-written rule where the call is not a copy**: 94.8% against 91.7% reproducing the oracle's queries, 10/30 against 7/30 on the answer. That reverses P13 and locates it — P13 measured a suite with one tool and every constant pre-printed, where asking IS copying. `results/P15-multitool-20260910/`. | [ran] |
| M44 | **But that suite's tools were MEMORISABLE and the control voids its headline.** With no tool layer the expert scores **27/30 asking nothing** — seven fluids x two properties is fourteen numbers, learnable many times over from 600 examples. **A suite whose tool calls can be recalled cannot price a tool layer.** Fix: a handbook drawn per case. And the killing arm was bought LAST, against the project's own rule. | [ran] |
| M45 | **The tripwire guard is FALSIFIED — P16 measured two families, not a boundary.** With P16's cut fixed and two never-used families, the tool layer's rejection rate reads **0.18 outside against 0.15 inside**, and the guard scores 0.62 where chance is 0.60. P16's 0.86 came from six signals chosen with the answers visible and a cut fitted on the same fifty points — its own brief said so. **Per-region promotion still has no guard and the cheap direction is exhausted.** Also: pooled rejection was 0.348 while the per-case mean was 0.18; a detector thresholds a case, so quoting the pooled figure would be true and misleading. `results/P18-confirm-20260910/`. | [ran] |
| M40 | **A transcript judge selects for failing QUIETLY.** It accepts **41% of wrong work that looks clean** and 7% of wrong work that shows an error, so a variant that cannot malform a call outranks one that is more often right. **Subtracting tool rejections is backwards** — a visible error is a signal, not a fault. `results/P19-…`. | [ran] |
| M41 | **The fix is a conjunction: model judge AND procedural check.** False acceptance falls **41% -> 2%** for six points of correct work, it orders 2/2 comparable pairs where each judge alone orders 1/2, and it comes out nearly calibrated (0.767 against a true 0.767). The model judge cannot tell clean-because-correct from clean-because-it-never-tried; the procedural check re-executes and does not care how it looks. `results/P20-fitness-20260911/`. **Two pairs, candidates not bred by a loop.** | [ran] |
| M42 | **`phys-0000` is a different problem in different family sets.** A dict keyed on case id alone let held-out statements overwrite in-region ones, the judge saw the wrong problem for 60 chains and called them all incorrect, and it read as "selection fails" — nearly killing S7 on a dictionary key. Caught only because P17 had measured the same judge at 0.97 recall on those records. | [ran] |
