...
| M25 | **It is NOT separable as a thing to SERVE.** Two independently trained LoRAs over the same projections interfere rather than compose: stacked 0/30 (11/30 cases with a rejected call), blended 0.5/0.5 0/30 (12/30). Halving the deltas did not help, so magnitude is not the cause. The chains keep the step names and the order and lose the content. The configuration that works is P7's **merged** adapter, 40/40 — at the retrain-the-pool price `ARCHITECTURE.md` §4 exists to avoid. | [ran] |
