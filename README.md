...
| **S9** | a region that is somebody's morning | ✅ **a self-contained QLoRA clears the gate — 84/113 = 0.743 against a bar of 0.655, exact p = 0.028**, with **393 calls and 0 refused**. It asked in 113 of 150 cases and skipped the 37 the listing already settles, so it learned *when* to ask. The base scores 0.345, the exact complement of its own bar, from answering NOT IMPORTANT to everything. **No composition, no shared kernel, no tournament.** And tool use is two capabilities that generalise differently — the base alone asks for nothing 150 times. A kernel trained on **fluid-mechanics** tools asks in **123 of 150** cases on email and is refused every time; a kernel trained on the **email names** is refused **0** times and asks in only **22**. The disposition transfers broadly and vaguely, the vocabulary precisely and narrowly, **and neither alone reaches the margin** |
