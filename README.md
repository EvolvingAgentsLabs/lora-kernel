...
| **S9** | a region that is somebody's morning | 🟢 **the disposition to ask is what a LoRA carries between domains** — the base alone asks for nothing 150 times and scores the complement of its own bar (39/113, p = 1.000). A protocol adapter trained on **fluid-mechanics** tools takes it to **127 calls in 123 of 150 cases** on email — and all 127 are refused, because it asks in the vocabulary it learned. **The behaviour transfers; the names do not** |
