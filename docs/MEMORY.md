...
**A site may also ADD one sentence to a note — never rewrite the textbook's line [spec] W5c.** A unit
whose rule the textbook does not have at all — typically a second value of a quantity under a condition —
states it as `adds:`, a sentence with slots of its own; the runtime appends it to the note and marks its
values `[site]` like any other (`memory/layers.py`, linted: every slot valued, none of the note's own
reused, the note still under its limit):
...
A site that could edit the source's words could make the library say what its source does not; one that
can only add is always distinguishable from it. The example site `unit-4c` is **invented content**, and
says so.
...
| W5 | **the kill arm** — two adapters on one subdomain, with and without the library, scored on a held-out sibling procedure, **as walks, not as questions**; beside them the *bare base with the oracle's notes open* | Colab, two sessions | with-library beats without, paired — **and beats the untrained base reading the same notes**, which on question-answering is already at 45/48 **[ran]** M5 — or the memory stops here at this model size — ❌ **[ran] 2026-09-19 — does not pass as written.** Headline n = 56: `withlib` **35**, `base-reads` **45**, `nolib` 2, `base-walks` 0. `withlib` vs `nolib` 35 : 2; vs `base-walks` 35 : 0; **vs `base-reads` 6 : 16, $p=0.052$ — a tie that leans to a loss**. 12 of the 21 failures are one: the held-out note states two values for one quantity, the corpus never showed such a note (0 of 108 quantity rows), and the adapter answers the first — 11/12 on the first value, 0/11 on the second, `base-reads` right on all 12. Navigation did transfer (0 retrieval misses, 3 refused verbs against 368; control 58/60 against 41/60). **Composition [ran] W5b** (the adapter walks, the bare base writes the last line, no training): **0 of those 12 remain**, `composed` 41/54 = its zero-GPU ceiling, a tie with `withlib` (12 : 4) and with `base-reads` (1 : 5) — and on what the adapter was taught the base reads worse (control 39 against 58), so it attributes the failure and is not a serving design; both two-valued notes in the library are in the held-out procedure ([`BRIEF`](../results/M7-W5b-composition-20260919/BRIEF.md)). Next, the user's call: two-valued notes inside trained procedures, retrain, a new held-out set. Built and pre-registered: `training/nursing/walks_arm.py`, four sessions with the untrained-base **headroom session first** (≥ 51/56 on the headline stops the step before any training); headline = held-out rows at depth ≤ 9 whose final line is the procedure's own, **n = 56**; trivial-policy floor **3/56** [ran], oracle 140/140 through the same function ([`BRIEF`](../results/M7-W5-kill-arm-20260919/BRIEF.md)) |
| W5c | **the corpus-shape arm** — W5 failed the conditional value of a two-valued note 0 of 11; corpus v2 shows that shape (one real note, `rates/drop-factor`, byte-checked; seven notes with a sentence a site ADDS — **invented example content**, marked) and asks each value half the time; NEW held-out and control sets | Colab, four sessions | ❌ **[ran] — FALSIFIED: conditional value 4/15** (0/11 before, Fisher $p=0.091$); W5's pair on the new headline 12 : 16, a tie, not passed; the adapter reads the condition on the eight notes it trained on (17/18) and writes the first number on an unseen one; navigation improved (shared line 22/22, control 78/80). *Built and pre-registered [ran, zero GPU]:* v2 gate PASSED — 600 rows, 153 conditional over 8 notes of both trained procedures, "copy the first number" worth 0.51, held-out note opened 0/600, 0 of 168 evaluation cases is one of W5's; floor 6/66; headroom stop 61/66. **Falsifier, exact:** credit on the conditional slice ≤ 4 of 15 (Fisher against 0/11, $p \ge 0.05$) → showing the shape did not teach reading it ([`BRIEF`](../results/M7-W5c-conditional-corpus-20260919/BRIEF.md)) |
