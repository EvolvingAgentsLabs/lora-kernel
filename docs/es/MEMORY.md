...
**Un sitio también puede AGREGAR una oración a una nota — nunca reescribir la línea del libro [spec] W5c.**
Una unidad cuya regla el libro directamente no tiene — típicamente un segundo valor de una cantidad bajo una
condición — la declara como `adds:`, una oración con slots propios; el runtime la agrega al final de la nota
y marca sus valores `[site]` como cualquier otro (`memory/layers.py`, con lint: todo slot con valor, ninguno
de los de la nota reutilizado, la nota todavía bajo su límite):
...
Un sitio que pudiera editar las palabras de la fuente podría hacerle decir a la biblioteca lo que su fuente
no dice; uno que sólo puede agregar siempre se distingue de ella. El sitio de ejemplo `unit-4c` es
**contenido inventado**, y lo dice.
...
| W3 | `memory/index.py`, radar **R0**, al lado de una línea de base léxica | Colab, minutos | recall@3 de la nota necesaria sobre las consultas del oráculo; la línea de base reportada al lado — ❌ **[ran] 2026-09-19 — no pasa: R0 le gana a un buscador por palabras y no alcanza.** Construido, y después corrido: las consultas de W2 acá son nulas (una nota sale primera para su propio `when:`), así que el radar se mide sobre un conjunto escrito después de congelar el diseño — **P**, una paráfrasis de cabecera por nota (94, solapamiento medio de palabras con el objetivo 0,039), la compuerta; **E**, los 72 enunciados → la primera nota del recorrido, al lado. Recall@3 de la base léxica: **0,064** en P, 0,125 en E — hay margen, y un piso por construcción, así que la compuerta lleva además un estándar absoluto: R0 gana el par **y** recall@3 ≥ 0,80. **R0 (`Qwen3-Embedding-0.6B`, una sesión de L4): recall@3 0,638 en P — 56 : 2 contra el léxico, y por debajo del 0,80 fijado antes de correr.** β = 0,5 le gana a sólo-`when:` 30 : 4. De 34 fallos, 13 están en rango 4–6 (recall@6 0,777, recall@10 0,830) y 16 más allá del 10; cuatro de esos se resuelven dentro de su estante. El estándar no se afloja: la brecha es de W6, sobre conjuntos nuevos, y W5 cuenta un fallo de recuperación aparte del puntaje del experto ([`BRIEF`](../../results/M7-W3-radar-r0-20260919/BRIEF.md)) |
| W4 | el generador de corpus: huecos inmemorizables, ids opacos, distractores, callejones sin salida, mezcla de profundidades | no | ningún valor de hueco en ningún enunciado; ningún recorrido evaluado en el corpus — ✅ **[ran] 2026-09-19**: `training/nursing/generate_walks.py` *maneja* el runtime — 600 filas (y los mismos 600 casos sin verbos, el segundo brazo de W5), 80 recorridos de evaluación del procedimiento apartado y 60 de control; **0** valores en un enunciado, **0** casos evaluados en el corpus, **0** aperturas de las 17 notas que son solo de `discontinue-iv`, **0** filas que el árbitro no reproduzca byte por byte en `strict`; cada cláusula rota por un test. Los recorridos largos son **ventanas** con el estado llevado (`Conversation.resume`); máximo 1 298 de 1 536 tokens **El evaluador se reemplazó antes de que W5 leyera a través de él** (rediseño 2; un tercero termina el paso): una revisión adversarial encontró que fallaba con una paráfrasis y aprobaba con un número señuelo; ahora lee sólo la línea final, un número sólo donde está pegado a su unidad, un paso por *atribución* entre las notas de paso, y el recorrido en el registro del árbitro — estados `right` · `format` · `unread` · `wrong` ([`BRIEF`](../../results/M7-W4-corpus-20260919/BRIEF.md)) |
| W5 | **el brazo que mata** — dos adaptadores sobre un subdominio, con y sin la biblioteca, puntuados sobre un procedimiento hermano retenido, **como recorridos, no como preguntas**; al lado, la *base pelada con las notas del oráculo abiertas* | Colab, dos sesiones | con-biblioteca le gana a sin-biblioteca, pareado — **y le gana a la base sin entrenar leyendo las mismas notas**, que en preguntas y respuestas ya está en 45/48 **[ran]** M5 — o la memoria se detiene acá a este tamaño de modelo — ❌ **[ran] 2026-09-19 — no pasa tal como está escrito.** Titular n = 56: `withlib` **35**, `base-reads` **45**, `nolib` 2, `base-walks` 0. `withlib` vs `nolib` 35 : 2; vs `base-walks` 35 : 0; **vs `base-reads` 6 : 16, $p=0{,}052$ — un empate que se inclina a derrota**. 12 de las 21 fallas son una sola: la nota retenida da dos valores para una cantidad, el corpus nunca mostró una nota así (0 de 108 filas de cantidad), y el adaptador responde el primero — 11/12 con el primer valor, 0/11 con el segundo, `base-reads` acierta las 12. La navegación sí se transfirió (0 fallos de recuperación, 3 verbos rechazados contra 368; control 58/60 contra 41/60). **Composición [ran] W5b** (el adaptador recorre, el base pelado escribe la última línea, sin entrenar): **quedan 0 de esas 12**, `composed` 41/54 = su techo sin GPU, empate con `withlib` (12 : 4) y con `base-reads` (1 : 5) — y en lo que al adaptador se le enseñó el base lee peor (control 39 contra 58), así que atribuye la falla y no es un diseño de servicio; las dos notas de dos valores de la biblioteca están en el procedimiento retenido ([`BRIEF`](../../results/M7-W5b-composition-20260919/BRIEF.md)). Sigue, a decisión del usuario: notas de dos valores dentro de procedimientos entrenados, reentrenar, un conjunto retenido nuevo. Construido y pre-registrado: `training/nursing/walks_arm.py`, cuatro sesiones con la **sesión de margen de la base sin entrenar primero** (≥ 51/56 en el titular detiene el paso antes de entrenar nada); titular = filas apartadas de profundidad ≤ 9 cuya línea final es propia del procedimiento, **n = 56**; piso de la política trivial **3/56** [ran], oráculo 140/140 por la misma función ([`BRIEF`](../../results/M7-W5-kill-arm-20260919/BRIEF.md)) |
| W5c | **el brazo de la forma del corpus** — W5 erró el valor condicional de una nota de dos valores 0 de 11; el corpus v2 muestra esa forma (una nota real, `rates/drop-factor`, verificada byte a byte; siete notas con una oración que un sitio AGREGA — **contenido de ejemplo inventado**, marcado) y pide cada valor la mitad de las veces; conjuntos retenido y de control NUEVOS | Colab, cuatro sesiones | ❌ **[ran] — FALSADO: valor condicional 4/15** (antes 0/11, Fisher $p=0{,}091$); el par de W5 sobre el titular nuevo 12 : 16, un empate, no pasa; el adaptador lee la condición en las ocho notas que entrenó (17/18) y en una nota nueva escribe el primer número; la navegación mejoró (línea compartida 22/22, control 78/80). *Construido y pre-registrado [ran, cero GPU]:* compuerta v2 PASÓ — 600 filas, 153 condicionales sobre 8 notas de los dos procedimientos entrenados, "copiar el primer número" vale 0,51, nota retenida abierta 0/600, 0 de 168 casos de evaluación es uno de los de W5; piso 6/66; corte de margen 61/66. **Falsador, exacto:** crédito en el corte condicional ≤ 4 de 15 (Fisher contra 0/11, $p \ge 0{,}05$) → mostrar la forma no enseñó a leerla ([`BRIEF`](../../results/M7-W5c-conditional-corpus-20260919/BRIEF.md)) |
...
| W9 | **enunciados atómicos** (§1.6) — páginas de enunciados, `<open>id§anchor</open>`, respuestas citadas verificadas mecánicamente; una wiki de distribuidora inventada generada por mundo; preguntas enciclopédicas y operativas de 1 a 3 saltos | una L4, sin entrenar; un LoRA de trayectoria (2 semillas) sólo si el brazo de margen dice que la navegación es la brecha | **el brazo a libro cerrado tiene que fallar** (≤ 10 % de aciertos) o el conjunto es nulo; después `base-walks` contra `base-reads` decide si hace falta un LoRA — ✅ **[ran] 2026-09-25: PASÓ** — el recorrido sin entrenar 0/40 (nunca escribe un verbo después de un resultado), el LoRA de trayectorias 35/40 en las dos semillas (35 : 0), 3 saltos 16/16, empate con el base que recibe los enunciados del oráculo ([`BRIEF`](../../results/M7-W9-atomic-statements-20260924/BRIEF.md)) |
