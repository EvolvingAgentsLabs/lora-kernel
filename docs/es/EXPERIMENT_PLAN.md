...
| **4** | el veredicto de orden por aceptación (M-α, M2): tres α por caso, SUPPORTED / FALSIFIED / UNRESOLVED-como-fracaso escritos antes (§7.4) | 3 | la tesis misma | **P55b se detuvo en M-target [ran]**: el 32B en modo corpus **227/240**, **232/240** una vez que el verificador dejó de rechazar `2023-01-05` por `January 5` (5 de sus 13 pérdidas eran formato; el check se borró); las 8 restantes son reales — se pierde `thread_history → inbox` y dice *no date*. Contra `g600` en 240/240: **0 : 8, $p = 0,008$** — el target sin entrenar queda resolublemente por debajo del experto entrenado, **por segunda vez** (triage: 0,746 contra 0,989). **No medido.** Contador de rediseños: 2 de 3. **Cerrado 2026-09-17 (decisión D): M2 no es comprable en ninguna suite que este proyecto pueda generar** — la ventana *target ≥ mejor experto entrenado* **y** *un orden no trivial de expertos graduados* no existe en ninguna, medido dos veces. El desenlace negativo es el entregable; el último rediseño se guarda para una suite con datos reales y verificador, si aparece |
| **5** | el producto con grupos reales (CASE-TEAM), `--prune` apagado como brazo de atribución; cada miembro nuevo entra por la puerta de la Fase 1 | 1 | 0,546 → 0,775 reproducido sobre tráfico nuevo, con la fracción que sale medida | **`RUNNING` desde 2026-09-17** — primera compra: el brazo de atribución de `--prune` sobre la superficie de 54 herramientas que OpenClaw manda de verdad (P59) |
