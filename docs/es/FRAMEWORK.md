...
| **búsqueda de notas** | un encoder estándar: recall@3 0,638 contra una vara de 0,80 fijada de antemano (por palabras: 0,064). **Y la consulta es la del adaptador:** ante una redacción nueva escribe una consulta de entrenamiento de otro tema, textual en 9 de 11 fallas, donde el mismo buscador por palabras, con el enunciado del pedido, lista la nota necesaria 16 de 16 | **[ran]** W3, W5d |
...
- **Corrida sobre un conjunto así, la partición no pasa — y dice dónde está la próxima pared [ran] W5d.**
  `policy vs withlib` 5 : 0, $p=0{,}0625$: un empate. Donde el recorrido del adaptador abrió la nota que da el
  valor, el base la lee bien 21 de 21 — *el problema de lectura está resuelto donde hay una nota para leer*.
  Las otras 11 filas de valor nunca llegaron a una nota, y no porque la búsqueda sea débil: ante una redacción
  nueva el adaptador emite una **consulta memorizada**, de otro tema. Lo que el entrenamiento daña, entonces, no
  es sólo una habilidad de lectura sino todo lo que en el recorrido hay que *componer a partir del pedido* en
  vez de seguirlo desde la biblioteca: la consulta al principio, la lectura al final. Lo que compra es lo que
  queda en el medio — moverse por un procedimiento.
...
| **D** | **una biblioteca por rol** | formato, lint, árbitro, búsqueda, y una división del trabajo que pase un test retenido | W1–W4 construidos; la búsqueda de W3 debajo de su vara; W5 no pasa; la política de respuesta **[ran]** W5d: lee 21/21 donde se abrió una nota, un empate en total | §4: **quién escribe la consulta** — la del adaptador está memorizada; quién lee — resuelto donde hay nota; el recall de la búsqueda; las dos habilidades enseñadas sobre muchas notas o dejadas a algo que no sea el adaptador | una sesión de L4, sin entrenar: el runtime hace la primera búsqueda con el enunciado del pedido (cero GPU dice que la nota queda listada 16/16); *se detiene si* el adaptador, con la nota correcta a la vista, igual abre otra en ≥ 6 de 11 |
...
1. **Decidir quién lee** (D). Una sesión de L4, sin entrenar: la partición por tipo de tarea sobre el
   ...
   **Pre-registrado el 2026-09-20**, las dos etapas en una sesión, la política congelada en un commit
   ...
   [`BRIEF`](../../results/M7-W5d-answer-policy-20260920/BRIEF.md). **[ran] W5d — se detuvo tal como estaba
   escrito:** 5 : 0, $p=0{,}0625$, un empate; el control aguanta. El base lee 21/21 donde el recorrido abrió una
   nota; las 11 a las que nunca llegó son la consulta memorizada del adaptador, no el buscador. **Sigue, una
   sola incógnita:** que la primera búsqueda salga del enunciado del pedido.
