...
| **6** | the route to `Qwen3.8-27B`: D2 (C18's mechanism, with the log) → D3 → D4 | **4 = SUPPORTED** | D2: `applied` on the identity gate | **blocked by design** — 4 closed without a verdict; not by the tokenizer, not by C18 — **and C18 itself is explained: D2 ✅ [ran] 2026-09-19, a naming mismatch, renamed adapter `applied`** |
...
### D2 — 2026-09-19 **[ran]** · C18 IS A NAMING MISMATCH: the same weights, renamed, are applied

One L4 session, `lora_matrix --rekey`, [`results/D2-rekey-20260918/`](../results/D2-rekey-20260918/BRIEF.md); the verdict table was written before the run.

| arm | G1 in process | served | real activation |
|---|---|---|---|
| control `Qwen2.5-3B-Instruct` | passed | **applied** | — (asked exactly as P33 asked it) |
| subject `Qwen3.5-4B`, as trained | passed (`lora_B` 17,937) | **not applied** — identical to the base | **0 of 178** modules |
| subject, 496 tensors renamed, **not retrained** | — | **applied** | **152 of 178** modules |

$$\text{applied}(K)=\{k\in K: m(k)\in M\}:\qquad |\text{applied}(K)| = 0 \ \text{of } 496,\qquad |\text{applied}(\rho(K))| = 496 \ \text{of } 496 .$$

P33's observation is reproduced and its reading is withdrawn: not *a serving-stack limit*, but tensor names. The 26 modules the renamed adapter leaves empty are `lm_head`, `embed_tokens` and 24 `conv1d` — none of them targeted. The linear-attention projections (`in_proj_qkv`, `in_proj_z`, `in_proj_a/b`, `out_proj`) are among the 152 that received weights.

**The instrument lied once, and the brief caught it.** The brief's consistency check predicted `modules_with_weights = 0` for the as-trained subject; the run reported **531**. vLLM activates three dummy LoRAs while profiling and each logs per module like a real one: 531 = 3 × 177 and the 181 misses are 3 + **178** — the real activation landed nowhere. The rekeyed log, which came home, splits as `(177, 1) × 3 + (152, 26)` **[ran]**; **the subject's own log stayed on the card, so its 0 of 178 is arithmetic on its totals, not a direct reading.** The counter now reports the last activation (`split_activations`, tested against the recorded log), and the chain should fetch every arm's log, not the last.

**What it does not say.** That a Qwen3.5 expert is *good* — the adapter is a 60-step toy and the gate asks only whether served text differs from the base. That each projection family contributes. Anything about the 27B. And it moves no base: every released member is trained on Qwen 2.5, D4 stays blocked on M2. What changed is the reason — moving to a 3.x drafter is a retraining cost, no longer an impossibility. Redesign counter for D2: 0 of the hypothesis; 1 fix of the counter.
