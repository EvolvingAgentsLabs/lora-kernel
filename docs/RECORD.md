...
| **…and retrained on the pool's new base it does not hold: the last line.** `fluids-full` on `Qwen3.5-4B`, same corpus, same recipe, corpus mode, same 90 cases | **80/90** against its own 90/90 — **0 : 10**, $p=0.002$, REGRESSION; 80 : 0 against the bare 4B (0/90, a floor of format); 21 : 7 against the frontier. All ten are venturi chains identical to the 3B's passing chains up to the final line, where the 4B writes `<answer>x</answer>` — a tag in 0 of 600 corpus rows — instead of the JSON line. The number verifies 10/10. **Not released; the region stays `out`** | M7 arm 0c |
...
| **A word-matcher cannot find a note from a question that does not use its words — the headroom the radar is for** — zero GPU, no model. 94 bedside paraphrases, one per note, written after the design was frozen; mean word overlap with the target 0.039. $\text{recall@}3 = \lvert\{q : \operatorname{rank}_q \le 3\}\rvert / \lvert Q\rvert$ | lexical recall@3 **0.064** (6/94), 56 queries match no note at all; on the 72 question stems 0.125. A floor by construction, so R0's gate also carries an absolute 0.80. R0's own number is the next row | M7-W3 (headroom) |
| **An off-the-shelf encoder finds the note a word-matcher cannot — and not often enough.** Radar R0, `Qwen3-Embedding-0.6B`, $s(n\mid q)=\langle e(q),e_{\text{when}}\rangle+0.5\langle e(q),e_{\text{what}}\rangle$, the same 94 paraphrases | recall@3 **0.638** against 0.064 — **56 : 2** paired — and under the **0.80** fixed before the run: **W3 not passed**. β = 0.5 beats `when:`-only 30 : 4. Of 34 misses, 13 at rank 4–6 (recall@10 0.830), 16 past rank 10. The standard stands; the gap is R1's, on new sets | M7-W3 (R0) |
| **A corpus of walks can be generated by driving the referee, with nothing in it to memorise and the sibling never walked** — zero GPU, no model. $V(r)\cap N(r)=\varnothing$ for every row, $V$ the values read off a slot or a `<calc>`, $N$ the numbers the statement states | 600 rows + the same 600 with no verbs; 80 held-out and 60 control walks. **0** values in a statement, **0** evaluated cases in the corpus, **0** opens of `discontinue-iv`'s 17 own notes, **0** rows not reproduced byte for byte in `strict`; each clause broken by a test. A 32-step walk does not fit 1 536 tokens: windows, state carried; longest row 1 298. **Whether a model learns the habit is W5** | M7-W4 |
| **The kill arm of the memory, run once: the library arm does not beat the untrained base that reads.** Two adapters on `Qwen3.5-4B`, with and without the library, scored as walks on a procedure never walked in training; headline n = 56 | `withlib` **35**, `base-reads` **45**, `nolib` 2, `base-walks` 0. 35 : 2 over no-library, 35 : 0 over the base navigating alone — **6 : 16 against the base handed the oracle's notes, $p=0.052$: not passed.** 12 of 21 failures are the second of two values on a note shape the corpus never showed (0 of 108 quantity rows): 11/12 on the first value, 0/11 on the second. Navigation transferred — 0 retrieval misses, 3 refused verbs in 140 walks against 368, control 58/60 against 41/60 | M7-W5 |
| **…and the failure attributed: the trained reader misses one shape its corpus never showed.** `withlib`'s recorded walks replayed, the bare base writes the final line (`composed`) | **0 of the 12** clean-walk quantity failures remain; `composed` 41/54 = its zero-GPU ceiling; 12 : 4 against `withlib` ($p=0.077$, tie), 1 : 5 against `base-reads`; `carry/middle-find` 8 of 8 remain. **Not a serving design:** on what the adapter was taught the base reads worse — control 39 against 58 (0 : 19), shared line 8 against 15 (0 : 7). Both two-valued notes in the library are in the held-out procedure; 0 of 108 quantity rows read two values. W5's verdict stands | M7-W5b |
| **The source itself has the shape exactly once outside the held-out procedure — and a corpus can be built that prices "copy the first number" at one half.** Open RN ch. 23, read line by line: no trained checklist states two values of one quantity; the chapter's prose does once (macro- against micro-drip drop factors, byte-checked against the page) | corpus v2, zero GPU: 153/600 rows ask one of a two-valued note's values — 30 on the real note, 123 on site-added sentences **declared invented** — conditional asked 75/153, first-number policy 0.51; held-out note opened 0/600; 0/168 new evaluation cases is one of W5's; floor 6/66. The adapters are **not trained yet** | M7-W5c (gate) |
| **…and showing the shape did not teach reading it — the diagnosis is falsified.** Corpus v2, same recipe, new held-out and control sets written after the freeze | conditional value **4/15** (0/11 before; one-sided Fisher $p=0.091$, band fixed beforehand). W5's pair on the new headline: `withlib` 42 vs `base-reads` 46, **12 : 16, $p=0.57$ — a tie, not passed**; 39 : 0 against no library, 42 : 0 against the base walking. 14 quantity failures, all clean walks, all right for the untrained base: the adapter writes the first number as one integer. On the eight notes it trained on it reads the condition 17/18 — it learned the notes, not the skill. Navigation improved: shared line 22/22 (base 8), control 78/80 (base 56) | M7-W5c |
...
| **The live peek clipped a verdict line.** The chain shows the last three matching lines of the run log; W5 printed four in a burst and `withlib vs nolib` never reached the streamed log | nothing was lost — the pairs were read from the JSON, as every verdict is — but a reader of the chain log alone would have seen two pairs of three. The log is for position; the file is for the verdict | M7-W5 |
